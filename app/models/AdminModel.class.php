...
class Admin extends User {
    ...
    function UpdateTeacher($username,$userEmail,$fullName,$pass,$userid,$salary)
    {
        global $connect;

        $stmt = $connect->prepare("UPDATE users SET userName = '$username', userEmail = '$userEmail', fullName = '$fullName', userPassword = '$pass' WHERE userID = '$userid'");
        $stmt2 = $connect->prepare("UPDATE teacher SET teacherName = '$username',salary ='$salary' WHERE teacherID = '$userid'");
        $stmt->execute();
        $stmt2->execute();
    }
